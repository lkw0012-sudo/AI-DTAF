...
def contains_unrelated_example_text(text: Any) -> bool:
    return bool(UNRELATED_EXAMPLE_RE.search(str(text or "")))
...
def sanitize_domain_text(text: Any, sector: str, problem_statement: str, replacement_hint: str = "") -> str:
    ...
    if not s:
        return ""
    if contains_unrelated_example_text(s):
        repl = build_use_case_example(sector, problem_statement, replacement_hint)
        return repl
    return s


def audit_score_with_evidence(score: Any, evidence: Any, rationale: Any) -> tuple[int, str, str]:
    try:
        score_int = int(score)
    except Exception:
        score_int = 0
    ...
    if contains_unrelated_example_text(evidence_s):
        evidence_s = ""
        score_int = min(score_int, 1)
    if contains_unrelated_example_text(rationale_s):
    ...
    for lid, lvl in levels.items():
        ...
        for gate in lvl.get('gate_scores', []):
            if not isinstance(gate, dict):
                continue
            score, evidence, rationale = audit_score_with_evidence(gate.get('score', 0), gate.get('evidence', ''), gate.get('rationale', ''))
...
def default_acceptance_for_dtv(text: str, current_target_level: str) -> str:
    ...
    if lvl == 'R1':
        if 'state' in txt:
            return 'State labels match the authoritative system log with the agreed accuracy for the selected representation fields.'
        if 'data' in txt or 'stream' in txt or 'mapping' in txt or 'schema' in txt:
            return 'Missing data rate is below the agreed threshold and every data stream is traceable to a component, state, or decision.'
        return 'Selected variables, schemas, and system boundaries are documented, traceable, and accepted as accurate enough for the intended decisions.'
    if 'timestamp' in txt or 'latency' in txt or 'sync' in txt:
        return 'Timestamp alignment is within the required update interval and data latency remains below the agreed limit.'
    if 'state' in txt:
        return 'State labels match the authoritative system log with the agreed accuracy.'
    if 'cycle' in txt or 'output' in txt or 'model' in txt or 'replic' in txt:
        return 'Replicated outputs match recorded system behavior within the agreed deviation threshold.'
...
def dtv_postprocess_result(result: Dict[str, Any], fourr_result: Dict[str, Any], sector: str, problem_statement: str, current_target_level: str) -> Dict[str, Any]:
    ...
    if not result.get('dtv_aligned_action_items') or any(contains_unrelated_example_text(x) for x in result.get('dtv_aligned_action_items', [])):
        regenerated = []
        for row in rows[:6]:
...
def build_fallback_4r_context_summary_for_dtv(fourr_result: Dict[str, Any]) -> str:
    if not isinstance(fourr_result, dict) or not fourr_result:
        return ""
    level = str(fourr_result.get("recommended_target_level", "")).strip() or str(fourr_result.get("current_supported_target_level", "")).strip() or str(fourr_result.get("highest_supported_target_level", "")).strip()
    name = str(fourr_result.get("recommended_target_name", "")).strip() or str(fourr_result.get("current_supported_target_name", "")).strip()
    ...
    pieces = []
    if level or name:
        target_txt = level if not name or name == level else f"{level} ({name})"
        pieces.append(f"The current supported 4R build target is {target_txt}.")
    if next_level and next_level != level:
        ...
        pieces.append(f"The near-term next target is {next_txt} once the remaining evidence and implementation gaps are closed.")
    if planning:
...
def run_dtv_assessment_addon(
    *,
    ...
    raw_obj = dtv_generate_guidance_with_ollama(
        base_url=base_url,
        model=model,
        timeout=timeout,
        retries=retries,
    ...
    current_target_level = get_current_4r_target_level_for_dtv(fourr_inputs.get("result", {}))
    result = dtv_validate_result(raw_obj)
